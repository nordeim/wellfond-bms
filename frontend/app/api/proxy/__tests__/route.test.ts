...
describe('BFF Proxy Path Validation (Critical Issue C1)', () => {
  describe('Valid paths (should allow)', () => {
    const validPaths = [
      ...
      { path: '/stream/alerts', reason: 'SSE stream endpoint' },
      { path: '/stream/heartbeat', reason: 'SSE stream sub-path' },
      { path: '/alerts/', reason: 'alerts endpoint' },
      { path: '/alerts/status', reason: 'alerts sub-path' },

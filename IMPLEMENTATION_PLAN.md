...
4. **Entity Scoping** — All queries filtered by `entity_id`. No cross-entity data leakage. Enforced at queryset level (not RLS).
...
| 2.6 | `backend/apps/operations/services/vaccine.py` | Vaccine due date calculator | `calc_vaccine_due(dog) -> date`. Standard intervals: 21 days (puppy series), 1 year (annual boosters). Overdue detection. Returns next due date + status. | ☐ Correct for standard vaccine schedules |
...
| 4.7 | `backend/apps/breeding/tasks.py` | Celery closure rebuild | `rebuild_closure_table(full_rebuild=True)` — TRUNCATE + recursive CTE insert. `rebuild_closure_table(full_rebuild=False)` — incremental for single-dog. Queue: low. Max retries: 2. | ☐ Full rebuild <8s for 483 dogs ☐ Incremental <1s |
...
| 7.6 | `backend/apps/customers/services/comms_router.py` | WA template manager + email fallback | `route_message(customer, template, payload)`. Caches WA template approval status. Falls back to Resend email on WA failure/rejection. Logs channel switch in CommunicationLog. | ☐ Fallback to email on WA failure ☐ Channel switch logged |
| 7.7 | `backend/apps/customers/tasks.py` | Celery fan-out | `dispatch_blast(blast_id)` — chunked dispatch (50 per chunk). Exponential retry. DLQ on 3 failures. Updates progress via Redis pub/sub. `log_delivery(customer_id, channel, status)` — per-message logging. | ☐ Fan-out works ☐ Progress updates live ☐ DLQ captures failures |
| 7.8 | `backend/apps/customers/admin.py` | Django admin | Customer, CommunicationLog, Segment. | ☐ Admin shows customers |
| 7.9 | `backend/apps/customers/tests/test_segmentation.py` | Segmentation tests | Filter combinations. PDPA exclusion. Cached counts. | ☐ All pass |
| 7.10 | `backend/apps/customers/tests/test_blast.py` | Blast tests | Mock Resend/WA APIs. Rate limiting. Bounce handling. PDPA enforcement. | ☐ All pass |
